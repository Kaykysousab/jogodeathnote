...
const coinImg = creatImage("images/death note.png");
...
const platformImg = creatImage("images/platform4.png");
const platformSmallImg = creatImage("images/plataform3.png");
const hillsImg = creatImage("images/hills2.png");
const backgroundImg = creatImage("images/background.png");
const goombaImg = creatImage("images/ryuk2.png");

const backgroundSound = createAudio("audio/backsond.mp3");
const coinSound = createAudio("audio/coletalv.mp3");
const killEnemySound = createAudio("audio/ryuk.mp3");
const gameOverByEnemySound = createAudio("audio/overenny.wav");
const gameOverByEnemySound2 = createAudio("audio/ryuk2.mp3");
const gameOverFallSound = createAudio("audio/overnormal.wav");
...
function initGame() {
    ...
    enemies = [
        new Enemy({x:platformImg.width * 1 + 000, y:390}, {s: platformImg.width * 1 + 000, e: platformImg.width * 1 + 500}),
        new Enemy({x:platformImg.width * 2 + 200, y:390}, {s: platformImg.width * 2 + 200, e: platformImg.width * 2 + 600}),
        new Enemy({x:platformImg.width * 3 + 300, y:390}, {s: platformImg.width * 3 + 300, e: platformImg.width * 3 + 700}),
        new Enemy({x:platformImg.width * 5 + 300, y:390}, {s: platformImg.width * 5 + 000, e: platformImg.width * 5 + 700}),
        new Enemy({x:platformImg.width * 9 + 300, y:390}, {s: platformImg.width * 9 + 000, e: platformImg.width * 9 + 600}),
        new Enemy({x:platformImg.width * 14 - 300, y:390}, {s: platformImg.width * 14 - 300, e: platformImg.width * 14 - 050}),
    ];
    ...
    enemies.forEach((enemy, index) => {
        ...
        if((dis < player.width || dis < enemy.width)
            ...
            && player.position.y + player.height <= enemy.position.y + 30){
                ...
                scoreEl[0].innerHTML = score;
                // Kill Enemy Sound
                killEnemySound2.play();
                killEnemySound2.volume = 0.1;
                // Create Explosions
                for (let i = 0; i < 15 ; i++){
                    particles.push(new Particle(
                        enemy.position.x + enemy.width / 2,
            ...
            && player.position.y + player.height + 50 >= enemy.position.y + enemy.height){
                ...
                if(!muted){
                    backgroundSound.pause();
                    gameOverByEnemySound2.play();
                    gameOverByEnemySound2.volume = 0.1;
                }
